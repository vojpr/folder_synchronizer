...
class FolderSynchronizer():
    ...
    def copy_and_update(self):
        """
        Checks which files or folders from the source folder are missing in the replica 
        folder and then creates their copies in the replica folder and compares the 
        modification time of files in the source folder and the replica folder
        ...
        for (root, dirs, files) in os.walk(self.source_folder):
            for unit in dirs + files:
                ...
                # If file/folder does not exist in replica folder -> create it
                ...
                # If file exists in replica folder and is outdated -> update it
                elif os.path.isfile(os.path.join(self.source_folder, rel_path)):
